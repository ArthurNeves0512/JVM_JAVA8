...
void executaFrame(Frame *f, ClassFile *cf) {
    ...
    while (f->pc < f->tamanho) {
        ...
        case 0x60: { int32_t b = pop_int(f); push_int(f, pop_int(f) + b); break; }
        case 0x64: { int32_t b = pop_int(f); push_int(f, pop_int(f) - b); break; }
        case 0x68: { int32_t b = pop_int(f); push_int(f, pop_int(f) * b); break; }
        case 0x6C: {
        ...
        case 0x78: { int32_t s = pop_int(f) & 0x1F; push_int(f, pop_int(f) << s); break; }
        case 0x7A: { int32_t s = pop_int(f) & 0x1F; push_int(f, pop_int(f) >> s); break; }
        case 0x7C: { int32_t s = pop_int(f) & 0x1F; push_int(f, (int32_t)((uint32_t)pop_int(f) >> s)); break; }
        case 0x7E: { int32_t b = pop_int(f); push_int(f, pop_int(f) & b); break; }
        case 0x80: { int32_t b = pop_int(f); push_int(f, pop_int(f) | b); break; }
        case 0x82: { int32_t b = pop_int(f); push_int(f, pop_int(f) ^ b); break; }
        ...
        case 0x99: { int16_t off = read_offset(f); if (pop_int(f) == 0) f->pc = inicio_op + (int32_t)off; break; }
        case 0x9A: { int16_t off = read_offset(f); if (pop_int(f) != 0) f->pc = inicio_op + (int32_t)off; break; }
        case 0x9B: { int16_t off = read_offset(f); if (pop_int(f) <  0) f->pc = inicio_op + (int32_t)off; break; }
        case 0x9C: { int16_t off = read_offset(f); if (pop_int(f) >= 0) f->pc = inicio_op + (int32_t)off; break; }
        case 0x9D: { int16_t off = read_offset(f); if (pop_int(f) >  0) f->pc = inicio_op + (int32_t)off; break; }
        case 0x9E: { int16_t off = read_offset(f); if (pop_int(f) <= 0) f->pc = inicio_op + (int32_t)off; break; }

        case 0x9F: { int16_t off = read_offset(f); int32_t b = pop_int(f), a = pop_int(f); if (a == b) f->pc = inicio_op + (int32_t)off; break; }
        case 0xA0: { int16_t off = read_offset(f); int32_t b = pop_int(f), a = pop_int(f); if (a != b) f->pc = inicio_op + (int32_t)off; break; }
        case 0xA1: { int16_t off = read_offset(f); int32_t b = pop_int(f), a = pop_int(f); if (a <  b) f->pc = inicio_op + (int32_t)off; break; }
        case 0xA2: { int16_t off = read_offset(f); int32_t b = pop_int(f), a = pop_int(f); if (a >= b) f->pc = inicio_op + (int32_t)off; break; }
        case 0xA3: { int16_t off = read_offset(f); int32_t b = pop_int(f), a = pop_int(f); if (a >  b) f->pc = inicio_op + (int32_t)off; break; }
        case 0xA4: { int16_t off = read_offset(f); int32_t b = pop_int(f), a = pop_int(f); if (a <= b) f->pc = inicio_op + (int32_t)off; break; }

        case 0xA7: {
            int16_t off = read_offset(f);
            f->pc = inicio_op + (int32_t)off;
            break;
        }

        case 0xBF: {
            fprintf(stderr, "athrow: excecao lancada (pc=%u)\n", inicio_op);
            exit(1);
        }

        case 0x57: f->topo--; break;
        case 0x59: f->topo++; f->pilha[f->topo] = f->pilha[f->topo - 1]; break;
        ...
        default:
            fprintf(stderr, "opcode 0x%02X nao implementado (pc=%u)\n", op, inicio_op);
            return;
        }
    }
}