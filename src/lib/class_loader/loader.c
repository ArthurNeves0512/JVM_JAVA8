...
void readPrintMinorMajorVersion(FILE *ptr_file) {
    ...
    printf("these are the major and minor version of the class file: %d.%d\n",
...
u2 readAcessModifiers(FILE *ptr_file) {
    ...
    e iremos fazer uma mascara para saber, quais são os modificadores.
    */
...
void constant_pool(u2 tagByte, FILE *ptr_file) {
    switch (tagByte) {
    case CONSTANT_Class:            read_CONSTANT_Class(ptr_file);            break;
    case CONSTANT_Fieldref:         read_CONSTANT_Fieldref(ptr_file);         break;
    case CONSTANT_Methodref:        read_CONSTANT_Methodref(ptr_file);        break;
    case CONSTANT_InterfaceMethodref: read_CONSTANT_InterfaceMethodref(ptr_file); break;
    case CONSTANT_String:           read_CONSTANT_String(ptr_file);           break;
    case CONSTANT_Integer:          read_CONSTANT_Integer(ptr_file);          break;
    case CONSTANT_Float:            read_CONSTANT_Float(ptr_file);            break;
    case CONSTANT_Long:             read_CONSTANT_Long(ptr_file);             break;
    case CONSTANT_Double:           read_CONSTANT_Double(ptr_file);           break;
    case CONSTANT_NameAndType:      read_CONSTANT_NameAndType(ptr_file);      break;
    case CONSTANT_Utf8:             read_CONSTANT_Utf8(ptr_file);             break;
    case CONSTANT_MethodHandle:     read_CONSTANT_MethodHandle(ptr_file);     break;
    case CONSTANT_MethodType:       read_CONSTANT_MethodType(ptr_file);       break;
    case CONSTANT_InvokeDynamic:    read_CONSTANT_InvokeDynamic(ptr_file);    break;
    default:                                                                   break;
    }
}
...
void readAcessFlags(ClassFile *clas_file_ptr, FILE *file_ptr) {
    ...
    if ((flag & 0x0001) == 0x0001)  printf("ACC_PUBLIC, ");
    if ((flag & 0x0010) == 0x0010)  printf("ACC_FINAL, ");
    if ((flag & 0x0020) == 0x0020)  printf("ACC_SUPPER, ");
    if ((flag & 0x0200) == 0x0200)  printf("ACC_INTERFACE, ");
    if ((flag & 0x0400) == 0x0400)  printf("ACC_ABSTRACT, ");
    if ((flag & 0x1000) == 0x1000)  printf("ACC_SYNTHETIC, ");
    if ((flag & 0x2000) == 0x2000)  printf("ACC_ANNOTATION, ");
    if ((flag & 0x4000) == 0x4000)  printf("ACC_ENUM, ");
    printf("\n");
...
void readSuperClass(ClassFile *class_file_ptr, FILE *file_ptr) {
    ...
    class_file_ptr->super_class = superClass;
}
...
void readInterfacesCount(ClassFile *class_file_ptr, FILE *file_ptr) {
    ...
    class_file_ptr->interfaces_count = interfacesCount;
}
...
void classFilesSetup(ClassFile *clas_file_ptr, FILE *file_ptr) {
    ...
    printf("Java class file version:%hu.%hu\n",
           clas_file_ptr->major_version, clas_file_ptr->minor_version);
