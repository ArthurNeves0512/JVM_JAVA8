...
u4 readCafeBabe(FILE * ptr_file){
    return u4Read(ptr_file);
}
static u2 readMinorVersion(FILE * ptr_file){
    return u2Read(ptr_file);
}
static u2 readMajorVersion(FILE* ptr_file){
    return u2Read(ptr_file);
}
...
void classFilesSetup(ClassFile * clas_file_ptr, FILE* file_ptr){
    ...
    /*
    No caso atual é 29, logo iremos ter que iterar de 0->28(ah nao ser que seja LONG ou Double, eai assim,
    iremos de 0->29)
    */
